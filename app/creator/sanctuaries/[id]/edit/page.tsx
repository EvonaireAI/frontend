...
export default function EditSanctuaryPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setUser(userData)
        const sanctuaryData = await sanctuariesService.getSanctuaryDetail(sanctuaryId)
        setSanctuary(sanctuaryData)
        setFormData({
          title: sanctuaryData.title,
          description: sanctuaryData.description,
          welcome_message: sanctuaryData.welcome_message,
          privacy: sanctuaryData.privacy,
          capacity: sanctuaryData.capacity,
          allow_open_join: sanctuaryData.allow_open_join,
          tags: sanctuaryData.tags.map((t) => t.name),
        })
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await sanctuariesService.updateSanctuary(sanctuaryId, {
      ...
      router.push(`/creator/sanctuaries/${sanctuaryId}`)
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to update sanctuary")
    } finally {
      setSubmitting(false)
    }
  }
      ...
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        <div className="mb-8">
          <Button asChild variant="outline" className="mb-4 bg-transparent">
            <Link href={`/creator/sanctuaries/${sanctuaryId}`}>
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            ...
            <CardDescription>Modify your sanctuary information</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex gap-2 pt-4">
                ...
                <Button type="button" variant="outline" asChild>
                  <Link href={`/creator/sanctuaries/${sanctuaryId}`}>Cancel</Link>
                </Button>
              </div>
            </form>
          </CardContent>
